...
export const projects: Project[] = [
    {
        ...
        description: {
            pt: 'Plataforma de gestão de anúncios digitais (Meta + Google Ads) potencializada por IA, com agentes autônomos e busca semântica de contexto via RAG + Vetorial Search',
            en: 'AI-powered digital ad management platform (Meta + Google Ads) with autonomous agents and semantic context search via RAG + Vetorial Search',
        },
        longDescription: {
            pt: 'O Löwe é o meu projeto mais recente e o mais ambicioso em IA: uma plataforma completa para gerenciar, otimizar e escalar campanhas de anúncios digitais no Meta e Google Ads, full TypeScript de ponta a ponta — backend em NestJS 11 e SPA em React 19 com Vite. Fui responsável por boa parte do sistema, do schema do banco até a UI: a camada de autenticação JWT com RBAC, os agentes de IA agendados (Auditor, Lances, Criativo e Analista) que rodam em paralelo gerando sugestões de otimização, o modelo de dados em Prisma 7 + PostgreSQL e o fluxo de aprovação human-in-the-loop no front. O coração de IA usa RAG (Retrieval-Augmented Generation): os dados de campanhas, criativos e histórico são transformados em embeddings e indexados com Vetorial Search dentro do próprio PostgreSQL, permitindo busca semântica de contexto — assim cada agente recupera exatamente o contexto relevante antes de gerar uma recomendação, tornando as sugestões muito mais precisas e fundamentadas. Redis cuida das sessões e o Socket.io entrega as atualizações em tempo real.',
            en: "Löwe is my most recent and most ambitious AI project: a complete platform to manage, optimize and scale digital ad campaigns on Meta and Google Ads, full TypeScript end to end — a NestJS 11 backend and a React 19 + Vite SPA. I owned a large part of the system, from the database schema up to the UI: the JWT auth layer with RBAC, the scheduled AI agents (Auditor, Bids, Creative and Analyst) that run in parallel generating optimization suggestions, the Prisma 7 + PostgreSQL data model and the human-in-the-loop approval flow on the front. The AI core uses RAG (Retrieval-Augmented Generation): campaign, creative and historical data are turned into embeddings and indexed with Vetorial Search inside PostgreSQL itself, enabling semantic context search — so each agent retrieves exactly the relevant context before generating a recommendation, making the suggestions far more accurate and grounded. Redis handles sessions and Socket.io delivers real-time updates.",
        },
        technologies: ['RAG', 'Vetorial Search', 'PostgreSQL', 'NestJS', 'CI/CD' ,'React 19', 'TypeScript', 'Prisma', 'Vite', 'Redis', 'Socket.io', 'JWT', 'Docker'],
        category: 'fullstack',
        featured: true,
        images: {
    ...
    },
    {
        ...
        technologies: ['AI Agent', 'Next', 'Nest', 'PrismaORM', 'MySQL', 'TypeScript', 'Docker', 'Git', 'Responsive Design'],
        category: 'fullstack',
        featured: false,
        images: {
